...
//           initialEmailSentState[user._id] = false;
//         });
...
//     try {
//       setLoadingUserId(userId); // Show loading state for this user
//       const token = localStorage.getItem("token");
...
//       if (response.data.success) {
//         // Update the emailSent state for this user
//         setEmailSent((prevState) => ({
//           ...prevState,
//           [userId]: true, // Mark as "Yes"
//         }));
//       } else {
//         alert("Failed to send invitation.");
//       }
//     } catch (error) {
//       alert(error.response ? "Failed to send invitation." : "Network error.");
//     } finally {
...
//                   >
//                     {loadingUserId === user._id ? "Sending..." : "No"}
//                   </button>
//                 </td>
...
//       if (response.data.success) {
//         console.log("Email sent:", userId); // Debug log
//         setEmailSent((prevState) => ({
//           ...prevState,
//           [userId]: true, // Mark as "Yes"
//         }));
...
const UserListing = () => {
  ...
  const sendInvite = async (userEmail) => {
    try {
      ...
      const response = await axios.post(
        "http://localhost:3000/api/admin/invite",
        { email: userEmail }, // Send email in the payload
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      if (response.data.success) {
        setEmailSent((prevState) => ({
          ...prevState,
          [userEmail]: true, // Mark email as sent
        }));
      } else {
        alert("Failed to send invitation.");
      }
